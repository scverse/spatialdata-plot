...
def test_align_outline_vector_no_op_on_match():
    from spatialdata_plot.pl.utils import _align_outline_vector_to_length

    vec = np.array(["#ff0000", "#00ff00", "#0000ff"], dtype=object)
    src = pd.Categorical(["a", "b", "a"])
    out_vec, out_src = _align_outline_vector_to_length(vec, src, n=3)
    assert out_vec is vec
    assert out_src is src


def test_align_outline_vector_truncate():
    from spatialdata_plot.pl.utils import _align_outline_vector_to_length

    vec = np.array(["#ff0000", "#00ff00", "#0000ff", "#000000"], dtype=object)
    src = pd.Categorical(["a", "b", "a", "b"])
    out_vec, out_src = _align_outline_vector_to_length(vec, src, n=2)
    assert len(out_vec) == 2
    assert len(out_src) == 2
    assert list(out_vec) == ["#ff0000", "#00ff00"]


def test_align_outline_vector_pad_continuous():
    from spatialdata_plot.pl.utils import _align_outline_vector_to_length

    vec = np.array([1.0, 2.0])
    out_vec, out_src = _align_outline_vector_to_length(vec, None, n=4)
    assert len(out_vec) == 4
    assert out_vec[0] == 1.0
    assert out_vec[1] == 2.0
    assert np.isnan(out_vec[2:]).all()
    assert out_src is None


def test_align_outline_vector_pad_categorical():
    from spatialdata_plot.pl.utils import _align_outline_vector_to_length

    vec = np.array(["#ff0000", "#00ff00"], dtype=object)
    src = pd.Categorical(["a", "b"])
    out_vec, out_src = _align_outline_vector_to_length(vec, src, n=4)
    assert len(out_vec) == 4
    assert len(out_src) == 4
    # padded source rows have no category
    assert out_src.codes[-1] == -1
    assert out_src.codes[-2] == -1
